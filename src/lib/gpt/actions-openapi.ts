...
export function buildGptActionsOpenApiSchema(
  ...
  return {
    ...
    paths: {
      ...
      "/api/gpt/memory/events": {
        get: {
          operationId: "searchSemanticMemory",
          summary: "Search semantically similar memories using pgvector.",
          parameters: [
            queryParam("q", "Search query for similar feedback, decisions or examples."),
            queryParam("campaign_id", "Optional campaign UUID or slug."),
            queryParam("company_id", "Optional company UUID."),
            queryParam("contact_id", "Optional contact UUID."),
            queryParam("source_type", "Optional repeated source type filter."),
            queryParam("limit", "Optional result count, max 25."),
          ],
          responses: { "200": okResponse },
          security,
          "x-openai-isConsequential": false,
        ...
        post: {
          operationId: "createSemanticMemory",
          summary: "Create a semantic memory event for future retrieval.",
          requestBody: jsonBody({
            type: "object",
            required: ["memory_text"],
            properties: {
              campaign_id: stringSchema("Optional campaign UUID or slug."),
              company_id: stringSchema("Optional company UUID."),
              contact_id: stringSchema("Optional contact UUID."),
              sender_account_id: stringSchema("Optional sender account UUID."),
              source_id: stringSchema("Optional source object UUID."),
              source_type: {
                type: "string",
                enum: [
                  "approved_message",
              ...
              memory_text: stringSchema("Reusable feedback, example, decision or context to remember."),
